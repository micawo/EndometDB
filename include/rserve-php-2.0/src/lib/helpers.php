...
// Since only php 5.6+ allows function importing from NS, functions are declared in global NS
// But prefixed by _rserve_
 ...
 * @param int $o offset
 */
function _rserve_int24($buf, $o = 0) {
 ...
 * @param int $o offset
 */
function _rserve_int32($buf, $o=0) {
 ...
 * @param $o
 */
function _rserve_flt64($buf, $o = 0) {
	$ss = substr($buf, $o, 8);
	if (Connection::$machine_is_bigendian) {
		for ($k = 0; $k < 7; $k++) { 
			$ss[7 - $k] = $buf[$o + $k];
		}	
	} 
...
function _rserve_make_packet($cmd, $data) {
	...
	return _rserve_mkint32($cmd) . _rserve_mkint32(strlen($data)) . _rserve_mkint32(0) . _rserve_mkint32(0).$data;
}
...
function _rserve_make_data($type, $string) {
	...
	$s .= _rserve_mkint24($len); // [1] Length (24bits)
	$s .= $string; // Data
	if($pad) {
